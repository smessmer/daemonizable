...
// `send_handshake` / `rpc_server_from_inherited_fd` are also used by
// `app::daemon_child`, so they stay crate-visible regardless of features; only
// their crate-root re-export in `lib.rs` is `testutils`-gated.
pub use spawn::{rpc_server_from_inherited_fd, send_handshake};

// Test-only surface. Internal code names the submodules directly, so these
// re-exports exist purely to feed the crate-root ones.
#[cfg(any(test, feature = "testutils"))]
pub use error::InheritedFdError;
// `in_process_rpc_pair`, not `RpcConnection`: keeping the connection type out of
// the re-export chain is what makes `new_channel` / `into_server_and_client`
// internal.
#[cfg(any(test, feature = "testutils"))]
