...
//! daemon child (via an in-band token on the channel fd, peeked before any app
//! code runs — nothing rides argv or the environment, so apps aren't forced
//! onto any particular argument parser), the fork+exec spawn, and the build-id
//! handshake. Everything else — CLI parsing, logging, panic hooks, banners — is
//! the application's business, inside [`Daemonizable::run_foreground`] and
//! [`Daemonizable::run_daemon`].
//!
