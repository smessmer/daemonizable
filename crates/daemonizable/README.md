...
pre-main constructor in your binary spawned a thread — see [`run`](https://docs.rs/daemonizable/latest/daemonizable/app/run/fn.run.html)'s
...
- [`Daemonizer::spawn_daemon`](https://docs.rs/daemonizable/latest/daemonizable/app/daemonizer/struct.Daemonizer.html#method.spawn_daemon) is safe to call with a tokio runtime already running —
  ...
  before any app code, so it is safe too — provided, as [`run`](https://docs.rs/daemonizable/latest/daemonizable/app/run/fn.run.html) documents,
