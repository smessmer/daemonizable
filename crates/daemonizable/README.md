...
Concretely, when `run_foreground` calls [`Daemonizer::spawn_daemon`](https://docs.rs/daemonizable/latest/daemonizable/app/daemonizer/struct.Daemonizer.html#method.spawn_daemon), the
library:
...
3. **Authenticate, then confirm the build.** Stage 2 consumes its own token,
   ...
   [`Daemonizer::spawn_daemon`](https://docs.rs/daemonizable/latest/daemonizable/app/daemonizer/struct.Daemonizer.html#method.spawn_daemon) returns only once the parent reads back a
...
From there the descriptor is simply your channel: [`Daemonizable::Request`](https://docs.rs/daemonizable/latest/daemonizable/app/daemonizable/trait.Daemonizable.html#associatedtype.Request) /
